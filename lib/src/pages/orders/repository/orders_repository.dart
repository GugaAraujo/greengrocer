...
class OrdersRepository {
  ...
  Future<OrdersResult<List<CartItemModel>>> getOrderItems({
    ...
    if(result['result'] != null) {
      List<CartItemModel> items = List<Map<String, dynamic>>.from(result['result'])
        .map(CartItemModel.fromJson)
        .toList();

      return OrdersResult<List<CartItemModel>>.success(items);
    } else {
      return OrdersResult.error('Não foi possível obter os itens do pedido');
    }
  }
  ...
  Future<OrdersResult<List<OrderModel>>> GetAllOrders({
    ...
    if (result['result'] != null) {
      ...
      return OrdersResult.error('Não foi possível obter os pedidos');
    }
  }
}
